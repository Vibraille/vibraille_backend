...
class TranslationSerializer(serializers.ModelSerializer):
    ...
    def create(self, data):
        ...
        try:
            ...
            #     new_note.save()
            # else:
            #     return Response(data="Invalid data format", status=status.HTTP_400_BAD_REQUEST)
            ...
            new_note.save()
            return Response(data=new_note.braille_format, status=status.HTTP_201_CREATED)
...
class NoteSerializer(serializers.ModelSerializer):
    title = serializers.CharField(max_length=100, required=False, allow_blank=True)
    img = serializers.ImageField(required=False, allow_empty_file=True, allow_null=True)
    ascii_text = serializers.CharField(required=False, allow_blank=True)
    braille_format = serializers.CharField(required=False, allow_blank=True)

    class Meta:
        model = Note
        fields = ['created', 'title', 'img', 'ascii_text', 'braille_format']
